...
* If running the react app as a standalalone configure the API_HOST and API_POST as follows inserting your host and port for the spring-boot server. Note, you will need to use the cors profile in this setup and potentially modify the cors config in the spring-boot app.

    API_HOST=http://127.0.0.1
    API_PORT=8080
    API_ROOT=api

* leave the host and port empty if running the React App bootstraped by spring-boot
