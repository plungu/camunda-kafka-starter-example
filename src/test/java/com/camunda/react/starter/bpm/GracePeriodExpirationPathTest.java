...
public class GracePeriodExpirationPathTest extends LeaseRenewalTestBase{
	...
	public void GracePeriodStaggerLessThan11DaysTillExpiration() {
		...
		assertTrue(WorkflowUtil.checkTaskCount(processEngine().getHistoryService(), processInstance.getId(), "getRemainingDays")>1);
		assertTrue(WorkflowUtil.checkTaskCount(processEngine().getHistoryService(), processInstance.getId(), "noticeDecision")>1);
		assertTrue(WorkflowUtil.checkTaskCount(processEngine().getHistoryService(), processInstance.getId(), "sendInitalTenantMessage")>1);
